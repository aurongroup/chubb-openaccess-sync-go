...
func newRecord(status, badgeType string, levels ...string) *model.AccessRecord {
	lvl := make([]string, 6)
	copy(lvl, levels)
	r, err := model.NewAccessRecord("", "", "Smith", lvl[0], lvl[1], lvl[2], lvl[3], lvl[4], lvl[5], "1", nil, nil, status, badgeType)
	if err != nil {
		panic(err)
	}
	return r
